...
/* create the thread */
pthread_create(&tid,&attr, (void * (*) (void *)) insert_to_front, (void * __restrict__) atoi(argv[1]));
...
void insert_to_front(int it)
 {
   ...
   new_rec->next = head;  /* link to the head */

   head = new_rec;   /* update the head */
}
